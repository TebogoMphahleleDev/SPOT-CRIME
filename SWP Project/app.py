...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL').replace("postgres://", "postgresql+psycopg://", 1)
...
class EmergencyAlert(db.Model):
    ...
    triggered_at = db.Column(db.DateTime(timezone=True), server_default=func.now())
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            officer = db.session.query(LawEnforcement).filter_by(email=email).first()
...
def law_enforcement_crime_map():
    if 'officer_id' not in session:
        ...
        return redirect(url_for('law_enforcement_login'))
    try:
        officer = db.session.query(LawEnforcement).filter_by(id=session['officer_id']).first()
        if not officer:
...
def admin_create_officer():
    ...
    try:
        ...
        existing = db.session.query(LawEnforcement).filter_by(email=email).first()
        if existing:
            return jsonify({'success': False, 'message': 'Email already exists'}), 400
...
def admin_get_officer(officer_id):
    if 'admin_id' not in session:
        return jsonify({'error': 'Not authorized'}), 401
    try:
        officer = db.session.query(LawEnforcement).filter_by(id=officer_id).first()
        if not officer:
...
def admin_update_officer(officer_id):
    ...
    try:
        ...
        if 'email' in data:
            existing = db.session.query(LawEnforcement).filter(LawEnforcement.email == data['email'], LawEnforcement.id != officer_id).first()
            if existing:
                return jsonify({'success': False, 'message': 'Email already exists'}), 400
...
def redeem_reward():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Reward redeemed successfully! Voucher pending approval.',
            'voucher_code': voucher_code,
